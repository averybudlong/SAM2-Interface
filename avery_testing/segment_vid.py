...
show_points(points, labels, plt.gca())
for i, out_obj_id in enumerate(out_obj_ids):
    show_points(*prompts[3], plt.gca())
    show_mask((out_mask_logits[i] > 0.0).cpu().numpy(), plt.gca(), obj_id=out_obj_id)
# plt.savefig("3.png")

# show the results on the current (interacted) frame
ann_frame_idx = 390  # the frame index we interact with
...
for out_frame_idx in range(0, len(frame_names), vis_frame_stride):
    ...
    # Add masks for this frame
    for out_obj_id, out_mask in video_segments[out_frame_idx].items():
        show_mask(out_mask, ax, obj_id=out_obj_id)
    
    # Save the figure
    plt.savefig(f"./out/{out_frame_idx:04d}.jpg", bbox_inches='tight', pad_inches=0, dpi=frame.width/fig.get_size_inches()[0])
